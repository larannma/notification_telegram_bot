...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_html(Constants.GREETINGS)
        ...
        states={
            ...
            ASK_NAME: [
                MessageHandler(
                    ...
                    set_notification_class.ask_name,
                )
            ],
            ASK_NOTIFICATION: [
                MessageHandler(
                    filters.TEXT & ~filters.COMMAND,
                    set_notification_class.ask_notification,
                )
            ],
            DATE_BUTTON_HANDLER: [
                CallbackQueryHandler(set_notification_class.date_button_handler)
            ],
            SET_MY_DATE: [
                MessageHandler(
                    filters.TEXT & ~filters.COMMAND,
                    set_notification_class.set_my_date,
                )
            ],
        },
