...
class SetNotification:
    ...
    async def ask_notification(
        ...
        await update.message.reply_text(
            "Choose the date when to sent your notification"
        )
        keyboard = [
            [
                InlineKeyboardButton(
                    "Sent after day", callback_data="Sent after day"
                )
            ],
            [
                InlineKeyboardButton(
                    "Sent after week", callback_data="Sent after week"
                )
            ],
            [
                InlineKeyboardButton(
                    "Choose my date", callback_data="Choose my date"
                )
            ],
        ]
        ...
        if query.data == "Sent after day":
            ...
            if temp is not None:
                database_class.insert_notification(
                    temp, context.user_data["text"], context.user_data["date"]
                )

            else:
                userId = database_class.insert_user(
                    context.user_data["tg_id"], context.user_data["name"]
                )
                database_class.insert_notification(
                    userId, context.user_data["text"], context.user_data["date"]
                )
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "Set Notification", callback_data="Set Notification"
                    )
                ],
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
